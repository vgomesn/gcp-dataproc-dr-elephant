...
function build() {
  ...
  git clone https://github.com/vgomesn/gcp-dataproc-dr-elephant.git /tmp/new-dr-elephant
 
  #pushd /tmp/dr-elephant
  mv /tmp/new-dr-elephant/dr-elephant /tmp/dr-elephant
  pushd /tmp/dr-elephant
  #git reset --hard ${DR_ELEPHANT_REVISION}

  # Install dependencies for new Dr. Elephant UI
  curl -sL https://deb.nodesource.com/setup_9.x | bash -
  apt-get install -y nodejs npm sbt
  sudo apt-get install -y nodejs npm sbt
  npm install -g bower --unsafe-perm=true --allow-root
  pushd web
